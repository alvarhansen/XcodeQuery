...
Context: After M6 GraphQL runtime migration, ensure runtime tests exercise all schema cases (both legacy executor and GraphQL runtime resolvers where applicable). Below is a concrete list of missing or lightly covered cases to implement as tests.

## Root Fields
- targets
  - Filter.name operators: eq, prefix, contains (suffix covered via snapshots).
  - GraphQL runtime execution path using `filter` (current tests use only `type`).
- target(name: String!)
  - Basic happy path via GraphQL runtime (baseline covered, Swift path not).
  - Error: unknown target (GraphQL runtime path).
- dependencies(name!, recursive=false, filter)
  - GraphQL runtime root resolver coverage (baseline covered).
...
- targetSources(pathMode=FILE_REF, filter)
  ...
  - Filter.target operator use (GraphQL runtime path).
...
- targetBuildScripts(filter)
  - GraphQL runtime root resolver coverage (baseline covered via legacy and CLI).
...
- targetMembership(path!, pathMode=FILE_REF)
  - GraphQL runtime coverage of membership lookup.
  - PathMode defaults (FILE_REF) and ABSOLUTE mode.

## Target Nested Fields
- Target.dependencies(recursive=false, filter)
  - GraphQL runtime nested with filter (type and name), and recursive flag.
...
- StringMatch operators matrix across fields: eq, regex, prefix, suffix, contains
  ...
  - Ensure at least one GraphQL runtime-backed test per operator and per filter kind.
...
## Error Handling (GraphQL runtime)
...
- Prefer reusing `GraphQLBaselineFixture` for deterministic projects and mirroring patterns in `GraphQL runtimeResolverTests` for GraphQL runtime execution.
...
- When adding GraphQL runtime error tests, use `graphql(schema:request:context:eventLoopGroup).wait()` and assert `result.data == nil` and `result.errors` non-empty.
